...
const YOUR_HANDLE = process.env.YOUR_HANDLE || '@your_insta_handle'; // Set a default
...
async function generateImage(backgroundImageUrl, text, author, fontPath) {
  ...
  image.brightness(-0.7); // Darken image for text readability
  ...
  if (author) {
    image.print(font, 0, -50, { // Position author near bottom
      text: `- ${author}`,
      alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER,
...
// ===================================================================
// INSTAGRAM POSTING ENGINE (Handles both single images and carousels)
// ===================================================================
async function postToInstagram({ items, caption }) {
  console.log(`Attempting to post ${items.length} item(s) to Instagram...`);
  ...
  if (items.length === 1) {
    ...
    // This part is for future use if you want to make Jimp carousels
...
function generateCaption(postData, handle, niche) {
    ...
    const nicheHashtags = hashtags[niche.toLowerCase()] || hashtags.default;
...
(async () => {
    try {
        ...
        // 1. Get the next post from our Google Sheet
        ...
        // 2. Load the configuration for the selected niche
        ...
        // 3. Find a background image from Pexels
        ...
        // 4. Generate the image using Jimp
        ...
        // 5. Post the final content to Instagram
        const caption = generateCaption(postData, YOUR_HANDLE, Niche);
        await postToInstagram({
            items: [{ file: imageBuffer }], // An array with our single image
            caption: caption
        });
